...
const STAKE_POOL_PROGRAM_ID = new web3_js.PublicKey('585UYiD9xQHgcrfty2PxW9GEUnQDKcAj931pKi9iv1pK');
...
class StakePoolInstruction {
    ...
    static updateStakePoolBalance(params) {
        ...
        const keys = [
            ...
            { pubkey: splToken.TOKEN_2022_PROGRAM_ID, isSigner: false, isWritable: false },
        ];
        return new web3_js.TransactionInstruction({
            programId: STAKE_POOL_PROGRAM_ID,
            keys,
...
async function depositStake(connection, stakePoolAddress, authorizedPubkey, validatorVote, depositStake, poolTokenReceiverAccount) {
    ...
    if (!poolTokenReceiverAccount) {
        const associatedAddress = splToken.getAssociatedTokenAddressSync(poolMint, authorizedPubkey, true, splToken.TOKEN_2022_PROGRAM_ID);
        let poolTokenReceiverInfo = await connection.getAccountInfo(associatedAddress);
        if (poolTokenReceiverInfo === null) {
            instructions.push(splToken.createAssociatedTokenAccountInstruction(authorizedPubkey, associatedAddress, authorizedPubkey, poolMint, splToken.TOKEN_2022_PROGRAM_ID));
            signers.push(web3_js.Keypair.generate());
        }
...
async function depositSol(connection, stakePoolAddress, from, lamports, destinationTokenAccount, referrerTokenAccount, depositAuthority) {
    ...
    if (!destinationTokenAccount) {
        const associatedAddress = splToken.getAssociatedTokenAddressSync(stakePool.poolMint, from, true, splToken.TOKEN_2022_PROGRAM_ID);
        let poolTokenReceiverInfo = await connection.getAccountInfo(associatedAddress);
        if (poolTokenReceiverInfo === null) {
            instructions.push(splToken.createAssociatedTokenAccountInstruction(from, associatedAddress, from, stakePool.poolMint, splToken.TOKEN_2022_PROGRAM_ID));
        }
...
async function withdrawStake(connection, stakePoolAddress, tokenOwner, amount, useReserve = false, voteAccountAddress, stakeReceiver, poolTokenAccount, validatorComparator) {
    ...
    if (!poolTokenAccount) {
        poolTokenAccount = splToken.getAssociatedTokenAddressSync(stakePool.account.data.poolMint, tokenOwner, true, splToken.TOKEN_2022_PROGRAM_ID);
    }
    const tokenAccount = await splToken.getAccount(connection, poolTokenAccount, "confirmed", splToken.TOKEN_2022_PROGRAM_ID);
...
async function withdrawSol(connection, stakePoolAddress, tokenOwner, solReceiver, amount, solWithdrawAuthority) {
    const stakePool = await getStakePoolAccount(connection, stakePoolAddress);
    const poolAmount = solToLamports(amount);
    const poolTokenAccount = splToken.getAssociatedTokenAddressSync(stakePool.account.data.poolMint, tokenOwner, true, splToken.TOKEN_2022_PROGRAM_ID);
    const tokenAccount = await splToken.getAccount(connection, poolTokenAccount, "confirmed", splToken.TOKEN_2022_PROGRAM_ID);
    // Check withdrawFrom balance
    if (tokenAccount.amount < poolAmount) {
        throw new Error(`Not enough token balance to withdraw ${lamportsToSol(poolAmount)} pool tokens.
...
async function createPoolTokenMetadata(connection, stakePoolAddress, payer, name, symbol, uri) {
    ...
    instructions.push(StakePoolInstruction.createTokenMetadata({
        stakePool: stakePoolAddress,
        poolMint: new web3_js.PublicKey("Es2RUioo4WvbhHdGSR65kgHscE4Q1jbmbJRQZEPcqmZ9"),
        payer,
        manager,
        tokenMetadata,
        withdrawAuthority,
