...
import { TOKEN_2022_PROGRAM_ID, getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, getAccount, createApproveInstruction } from '@solana/spl-token';
import { PublicKey, LAMPORTS_PER_SOL, StakeProgram, Keypair, SystemProgram, SYSVAR_CLOCK_PUBKEY, SYSVAR_STAKE_HISTORY_PUBKEY, TransactionInstruction, SYSVAR_RENT_PUBKEY, STAKE_CONFIG_ID, StakeAuthorizationLayout } from '@solana/web3.js';
...
const STAKE_POOL_PROGRAM_ID = new PublicKey('585UYiD9xQHgcrfty2PxW9GEUnQDKcAj931pKi9iv1pK');
...
async function depositStake(connection, stakePoolAddress, authorizedPubkey, validatorVote, depositStake, poolTokenReceiverAccount) {
    ...
    if (!poolTokenReceiverAccount) {
        ...
        if (poolTokenReceiverInfo === null) {
            instructions.push(createAssociatedTokenAccountInstruction(authorizedPubkey, associatedAddress, authorizedPubkey, poolMint, TOKEN_2022_PROGRAM_ID));
...
async function depositSol(connection, stakePoolAddress, from, lamports, destinationTokenAccount, referrerTokenAccount, depositAuthority) {
    ...
    if (!destinationTokenAccount) {
        ...
        let poolTokenReceiverInfo = await connection.getAccountInfo(associatedAddress);
        if (poolTokenReceiverInfo === null) {
            instructions.push(createAssociatedTokenAccountInstruction(from, associatedAddress, from, stakePool.poolMint, TOKEN_2022_PROGRAM_ID));
        }
...
async function withdrawStake(connection, stakePoolAddress, tokenOwner, amount, useReserve = false, voteAccountAddress, stakeReceiver, poolTokenAccount, validatorComparator) {
    ...
    instructions.push(createApproveInstruction(poolTokenAccount, userTransferAuthority.publicKey, tokenOwner, poolAmount, [], TOKEN_2022_PROGRAM_ID));
...
async function withdrawSol(connection, stakePoolAddress, tokenOwner, solReceiver, amount, solWithdrawAuthority) {
    const stakePool = await getStakePoolAccount(connection, stakePoolAddress);
    const poolAmount = solToLamports(amount);
    const poolTokenAccount = getAssociatedTokenAddressSync(stakePool.account.data.poolMint, tokenOwner, true, TOKEN_2022_PROGRAM_ID);
    const tokenAccount = await getAccount(connection, poolTokenAccount, "confirmed", TOKEN_2022_PROGRAM_ID);
    // Check withdrawFrom balance
    if (tokenAccount.amount < poolAmount) {
        throw new Error(`Not enough token balance to withdraw ${lamportsToSol(poolAmount)} pool tokens.
...
export { STAKE_POOL_INSTRUCTION_LAYOUTS, STAKE_POOL_PROGRAM_ID, StakePoolInstruction, createPoolTokenMetadata, decreaseValidatorStake, depositSol, depositStake, getStakeAccount, getStakePoolAccount, getStakePoolAccounts, increaseValidatorStake, redelegate, stakePoolInfo, tokenMetadataLayout, updatePoolTokenMetadata, updateStakePool, withdrawSol, withdrawStake };
